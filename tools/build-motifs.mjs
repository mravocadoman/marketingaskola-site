...
if (only !== '--pages-only') {
  ...
  for (const f of fs.readdirSync('src/posts').filter((f) => f.endsWith('.md'))) {
    ...
    if (photographic(img)) { photo.push(slug); continue; }

    if (!trace(img, `post-${slug}`, `Raksta motīvs: ${title.slice(0, 60)}`)) continue;

    /* The gate. A post that fails it is simply left out of the map, and
     * post.njk's `{% elif image %}` renders the cover as a picture instead -
     * a still image that looks right beats an animated one that does not. */
    const { pct, kept } = await fidelity(img, `post-${slug}`);
    if (pct > 3 || kept < 0.8 || kept > 1.3) {
      fs.unlinkSync(path.join(OUT, `post-${slug}.njk`));
      rejected.push({ slug, img, pct, kept });
      continue;
    }
  ...
  if (photo.length) {
    console.log(`  ${photo.length} kept their raster cover (photographic, never traceable):`);
    for (const sl of photo) console.log(`    ${sl}`);
  }
  if (rejected.length) {
    console.log(`  ${rejected.length} kept their raster cover (the trace lost the artwork):`);
