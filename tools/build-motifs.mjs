/* Build every motif on the site from PIECES of its own original artwork.
 *
 ...
 * new. I like the old style so just use and animate it." And on 11 Sep, after
 * the blog headers and the service pages had moved to pieces: "yes, one system".
 *
 * So nothing here is drawn by hand, and there is one method for everything:
 * each include is the real picture split into its own objects and reassembled
 * in place (see PIECES below). Tracing to flat colours is retired - it could not
 * keep a photograph, a hairline or a dot field, and finished on a fragment.
 *
...
// page motifs: [source picture, include name]. The four band-* rasters that
// once could not be traced (anti-aliased line work, a dot field) are back:
// pieces keep them exactly.
const PAGES = [
...
if (only !== '--posts-only') {
  ...
  const paged = [];
  for (const [srcId, name] of PAGES) { paged.push({ name, ...(await pieces(srcId, name, altOf(srcId))) }); ok++; }
...
if (only !== '--pages-only') {
  ...
  fs.writeFileSync('src/_data/postMotifs.json',
    JSON.stringify({ _comment: 'Generated by tools/build-motifs.mjs — each post is assembled from pieces of its own cover.', bySlug: merged }, null, 2) + '\n');
