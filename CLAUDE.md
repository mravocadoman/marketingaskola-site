...
- **One template, issues as JSON.** `npm run newsletter -- newsletters/<issue>.json`
  ...
  unsubscribe merge tag in the footer. **Sender reads Liquid tags,
  `{{ unsubscribe_link }}`:** a legacy `{$unsubscribe_link}` is stored as-is and
  reaches subscribers as literal text, so the renderer refuses any `{$` token.
...
- **marketingaskola.lv passes SPF, DKIM and DMARC in Sender since 15 Sep 2026**
  ...
  - TXT `google._domainkey` = Google Workspace's 2048-bit DKIM key, generated
    by the owner in Google Admin and published the same day
...
- **Found alongside, 15 Sep 2026: Google Workspace was missing from the
  ...
  default `google._domainkey` selector. The merged SPF record adds the Google
  include, and the DKIM key from Google Admin (Gmail, Authenticate email) is
  published at `google._domainkey` since the same day. Gmail only signs with it
  after "Start authentication" is clicked there, which takes the Workspace
  admin's own sign-in (a passkey), so it is the owner's click.
